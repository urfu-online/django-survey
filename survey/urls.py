...
urlpatterns = [
    ...
    path("csv/<primary_key:int>/", serve_result_csv, name="survey-result"),
    path("<slug:slug>/completed/", xframe_options_exempt(SurveyCompleted.as_view()), name="survey-completed"),
    path("<slug:slug>-<step:int>/", xframe_options_exempt(SurveyDetail.as_view()), name="survey-detail-step"),
